...
import { signInWithGitHubPopup } from "../../lib/firebaseClient";

interface Props {
  onLogin?: (user: { email?: string; name?: string }, idToken?: string) => void;
}

export default function SignInWithGitHub({ onLogin }: Props) {
  const handleSignIn = async () => {
    try {
      ...
      alert(`GitHub sign-in failed: ${message}`);
    }
  };
  ...
  return (
    <button
      ...
      className="w-full inline-flex items-center justify-center py-3 px-6 rounded-lg font-medium"
      style={{
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = '4px 4px 8px var(--shadow-color), -4px -4px 8px var(--highlight-color)';
      }}
      onMouseLeave={(e) => {
        e.currentTarget.style.boxShadow = '6px 6px 12px var(--shadow-color), -6px -6px 12px var(--highlight-color)';
      }}
    >
